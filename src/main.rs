...
fn main() -> anyhow::Result<()> {
    ...
    let active_tools: Vec<Box<dyn Tool>> = vec![Box::from(DFColorTool)];

    let output_dir = Path::new("./output");
    util::remove_dir_if_exists(output_dir)?;
    ...
    util::remove_dir_if_exists(dist_dir)?;

    util::copy_all_from_dir(output_dir, dist_dir)?;
    util::copy_all_from_dir(static_dir, dist_dir)?;
